...
//                System.out.println("matches:-》charSequence: "+charSequence.toString()+" ,S: "+s);
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests()
                // 如果有允许匿名的url，填在下面
                .antMatchers("/regis","/register","/test","/user/findUserList","/sysUserInfo/findLists",
                        "/sysUserInfo/findUserInfoList",
                        "/user/finds",
                        "/department/findDeptAndCount").permitAll()
                .anyRequest().authenticated()
                .and()
                // 设置登陆页 http://localhost:8080/#/
